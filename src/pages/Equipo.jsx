...
export default function Equipo({
  ...
  async function loadTeam() {
    try {
      ...
      const [managerData, invitationData] = await Promise.all([
        getWorkspaceManagers(workspaceId),
        getWorkspacePendingInvitations(workspaceId),
      ]);
      ...
      console.error(error);
      toast.error(
        error.message || 'No se pudo cargar el equipo.'
      );
    } finally {
      setLoading(false);
    }
      ...
      const invitation = await createWorkspaceInvitation(
        {
          ...
          commissionPercentage:
            inviteForm.commissionPercentage,
        },
        workspaceId
      );
  ...
  function beginCommissionEdit(manager) {
    setEditingCommissionId(manager.membership_id);
    setCommissionDraft(
      String(manager.commission_percentage || 0)
    );
  }

  async function saveCommission(manager) {
    try {
  ...
  async function removeManager(manager) {
    ...
    try {
      ...
      toast.success('Acceso del Gestor retirado.');
      await loadTeam();
    } catch (error) {
      console.error(error);
      toast.error(
    ...
    <div className="dashboard team-page">
      <div className="team-page-heading">
        <div>
          ...
          <h1>Equipo y Gestores</h1>
          <p>
            Controla quién puede trabajar con{' '}
            <strong>{workspace?.workspace_name}</strong> y qué
            comisión recibe.
          </p>
        </div>
      ...
      <section className="team-invite-panel">
        <div>
          <span className="team-eyebrow">Nuevo acceso</span>
          <h2>Invitar un Gestor</h2>
          <p>
            MiBooking enviará un correo profesional con un enlace seguro. La invitación solo podrá aceptarse desde una cuenta registrada con ese mismo correo.
          </p>
        </div>
      ...
      <section className="team-section">
        <div className="team-section-title">
          <div>
            <span className="team-eyebrow">Acceso activo</span>
            <h2>Gestores de {workspace?.workspace_name}</h2>
          </div>

          <span className="team-count">{managers.length}</span>
        </div>
        ...
        {loading ? (
          ...
          <div className="team-empty-state">
            <strong>Todavía no tienes Gestores activos.</strong>
            <span>
              Crea una invitación para vincular la primera cuenta.
            </span>
          </div>
        ) : (
            ...
            {managers.map((manager) => (
              ...
              >
                <div className="team-avatar">
                  {String(
                    manager.full_name || manager.email || 'G'
                  )
                    .slice(0, 1)
                    .toUpperCase()}
                </div>
                ...
                <div className="team-manager-info">
                  ...
                  <p>{manager.email}</p>
                  <small>
                    Vinculado desde {formatDate(manager.joined_at)}
                  </small>
                </div>

                <div className="team-manager-status">
                  <span
                    className={
                ...
                <div className="team-commission-control">
                  <span>Comisión</span>
                  ...
                  manager.membership_id ? (
                    <div>
                      <input
                        ...
                        onChange={(event) =>
                          setCommissionDraft(event.target.value)
                        }
                      />

                      <button
                        type="button"
                        onClick={() => saveCommission(manager)}
                      >
                        Guardar
                      </button>
                    ...
                    <button
                      ...
                      onClick={() =>
                        beginCommissionEdit(manager)
                      }
                    >
                      {Number(
                        manager.commission_percentage || 0
                      )}
                      %
                      <small>Editar</small>
                    </button>
      ...
      <section className="team-section">
        <div className="team-section-title">
          <div>
            <span className="team-eyebrow">En espera</span>
